...
export default class Page6 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page6.getRoleMenuTree().then(() => page6.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page6.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page6 } } = this.props;
      const searchRole = page6.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page6.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page6 } } = this.props;
    page6.setAddModalVisible(true);
    page6.setDisable(true);
    page6.setActiveKey('tab1');
    page6.setAddInputRole('');
    page6.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page6.deleteRole({ roleId: roleId })
          ]).then(() => {
            page6.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page6 } } = this.props;
    page6.setEditModalVisible(true);
    page6.setSaveBtnDisabled(true);
    page6.setActiveKey('tab1');
    page6.setAddInputRole(record.name);
    page6.setAddInputDes(record.describe);
    page6.setRoleId(record.roleId);
    page6.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page6.getRoleMenuTree({ roleId: record.roleId }).then(() => page6.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page6 } } = this.props;
    page6.setDetailModalVisible(true);
    page6.setDetailData(record.users);
  }
    ...
    return (
      <div className="page6">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page6.tableData)}
          bordered />
