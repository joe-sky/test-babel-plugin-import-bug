...
export default class Page7 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page7.getRoleMenuTree().then(() => page7.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page7.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page7 } } = this.props;
      const searchRole = page7.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page7.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page7 } } = this.props;
    page7.setAddModalVisible(true);
    page7.setDisable(true);
    page7.setActiveKey('tab1');
    page7.setAddInputRole('');
    page7.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page7.deleteRole({ roleId: roleId })
          ]).then(() => {
            page7.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page7 } } = this.props;
    page7.setEditModalVisible(true);
    page7.setSaveBtnDisabled(true);
    page7.setActiveKey('tab1');
    page7.setAddInputRole(record.name);
    page7.setAddInputDes(record.describe);
    page7.setRoleId(record.roleId);
    page7.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page7.getRoleMenuTree({ roleId: record.roleId }).then(() => page7.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page7 } } = this.props;
    page7.setDetailModalVisible(true);
    page7.setDetailData(record.users);
  }
    ...
    return (
      <div className="page7">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page7.tableData)}
          bordered />
