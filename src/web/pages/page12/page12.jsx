...
export default class Page12 extends Component {
  ...
  componentDidMount() {
    const { store: { page12 } } = this.props;

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page12.getRoleManagementData(),
      page12.getRoleMenuTree().then(() => page12.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page12.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page12 } } = this.props;
      const searchRole = page12.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page12.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page12 } } = this.props;
    page12.setAddModalVisible(true);
    page12.setDisable(true);
    page12.setActiveKey('tab1');
    page12.setAddInputRole('');
    page12.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page12.deleteRole({ roleId: roleId })
          ]).then(() => {
            page12.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page12 } } = this.props;
    page12.setEditModalVisible(true);
    page12.setSaveBtnDisabled(true);
    page12.setActiveKey('tab1');
    page12.setAddInputRole(record.name);
    page12.setAddInputDes(record.describe);
    page12.setRoleId(record.roleId);
    page12.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page12.getRoleMenuTree({ roleId: record.roleId }).then(() => page12.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page12 } } = this.props;
    page12.setDetailModalVisible(true);
    page12.setDetailData(record.users);
  }
    ...
    return (
      <div className="page12">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page12.tableData)}
          bordered />
