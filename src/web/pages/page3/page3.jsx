...
export default class Page3 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page3.getRoleMenuTree().then(() => page3.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page3.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page3 } } = this.props;
      const searchRole = page3.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page3.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page3 } } = this.props;
    page3.setAddModalVisible(true);
    page3.setDisable(true);
    page3.setActiveKey('tab1');
    page3.setAddInputRole('');
    page3.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page3.deleteRole({ roleId: roleId })
          ]).then(() => {
            page3.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page3 } } = this.props;
    page3.setEditModalVisible(true);
    page3.setSaveBtnDisabled(true);
    page3.setActiveKey('tab1');
    page3.setAddInputRole(record.name);
    page3.setAddInputDes(record.describe);
    page3.setRoleId(record.roleId);
    page3.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page3.getRoleMenuTree({ roleId: record.roleId }).then(() => page3.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page3 } } = this.props;
    page3.setDetailModalVisible(true);
    page3.setDetailData(record.users);
  }
    ...
    return (
      <div className="page3">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page3.tableData)}
          bordered />
