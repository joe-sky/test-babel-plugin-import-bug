...
export default class Page17 extends Component {
  ...
  componentDidMount() {
    const { store: { page17 } } = this.props;

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page17.getRoleManagementData(),
      page17.getRoleMenuTree().then(() => page17.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page17.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page17 } } = this.props;
      const searchRole = page17.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page17.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page17 } } = this.props;
    page17.setAddModalVisible(true);
    page17.setDisable(true);
    page17.setActiveKey('tab1');
    page17.setAddInputRole('');
    page17.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page17.deleteRole({ roleId: roleId })
          ]).then(() => {
            page17.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page17 } } = this.props;
    page17.setEditModalVisible(true);
    page17.setSaveBtnDisabled(true);
    page17.setActiveKey('tab1');
    page17.setAddInputRole(record.name);
    page17.setAddInputDes(record.describe);
    page17.setRoleId(record.roleId);
    page17.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page17.getRoleMenuTree({ roleId: record.roleId }).then(() => page17.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page17 } } = this.props;
    page17.setDetailModalVisible(true);
    page17.setDetailData(record.users);
  }
    ...
    return (
      <div className="page17">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page17.tableData)}
          bordered />
