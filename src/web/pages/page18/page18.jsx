...
export default class Page18 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page18.getRoleMenuTree().then(() => page18.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page18.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page18 } } = this.props;
      const searchRole = page18.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page18.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page18 } } = this.props;
    page18.setAddModalVisible(true);
    page18.setDisable(true);
    page18.setActiveKey('tab1');
    page18.setAddInputRole('');
    page18.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page18.deleteRole({ roleId: roleId })
          ]).then(() => {
            page18.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page18 } } = this.props;
    page18.setEditModalVisible(true);
    page18.setSaveBtnDisabled(true);
    page18.setActiveKey('tab1');
    page18.setAddInputRole(record.name);
    page18.setAddInputDes(record.describe);
    page18.setRoleId(record.roleId);
    page18.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page18.getRoleMenuTree({ roleId: record.roleId }).then(() => page18.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page18 } } = this.props;
    page18.setDetailModalVisible(true);
    page18.setDetailData(record.users);
  }
    ...
    return (
      <div className="page18">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page18.tableData)}
          bordered />
