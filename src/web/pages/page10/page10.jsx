...
export default class Page10 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page10.getRoleMenuTree().then(() => page10.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page10.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page10 } } = this.props;
      const searchRole = page10.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page10.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page10 } } = this.props;
    page10.setAddModalVisible(true);
    page10.setDisable(true);
    page10.setActiveKey('tab1');
    page10.setAddInputRole('');
    page10.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page10.deleteRole({ roleId: roleId })
          ]).then(() => {
            page10.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page10 } } = this.props;
    page10.setEditModalVisible(true);
    page10.setSaveBtnDisabled(true);
    page10.setActiveKey('tab1');
    page10.setAddInputRole(record.name);
    page10.setAddInputDes(record.describe);
    page10.setRoleId(record.roleId);
    page10.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page10.getRoleMenuTree({ roleId: record.roleId }).then(() => page10.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page10 } } = this.props;
    page10.setDetailModalVisible(true);
    page10.setDetailData(record.users);
  }
    ...
    return (
      <div className="page10">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page10.tableData)}
          bordered />
