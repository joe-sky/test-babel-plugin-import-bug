...
export default class Page9 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page9.getRoleMenuTree().then(() => page9.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page9.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page9 } } = this.props;
      const searchRole = page9.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page9.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page9 } } = this.props;
    page9.setAddModalVisible(true);
    page9.setDisable(true);
    page9.setActiveKey('tab1');
    page9.setAddInputRole('');
    page9.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page9.deleteRole({ roleId: roleId })
          ]).then(() => {
            page9.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page9 } } = this.props;
    page9.setEditModalVisible(true);
    page9.setSaveBtnDisabled(true);
    page9.setActiveKey('tab1');
    page9.setAddInputRole(record.name);
    page9.setAddInputDes(record.describe);
    page9.setRoleId(record.roleId);
    page9.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page9.getRoleMenuTree({ roleId: record.roleId }).then(() => page9.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page9 } } = this.props;
    page9.setDetailModalVisible(true);
    page9.setDetailData(record.users);
  }
    ...
    return (
      <div className="page9">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page9.tableData)}
          bordered />
