...
export default class Page19 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page19.getRoleMenuTree().then(() => page19.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page19.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page19 } } = this.props;
      const searchRole = page19.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page19.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page19 } } = this.props;
    page19.setAddModalVisible(true);
    page19.setDisable(true);
    page19.setActiveKey('tab1');
    page19.setAddInputRole('');
    page19.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page19.deleteRole({ roleId: roleId })
          ]).then(() => {
            page19.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page19 } } = this.props;
    page19.setEditModalVisible(true);
    page19.setSaveBtnDisabled(true);
    page19.setActiveKey('tab1');
    page19.setAddInputRole(record.name);
    page19.setAddInputDes(record.describe);
    page19.setRoleId(record.roleId);
    page19.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page19.getRoleMenuTree({ roleId: record.roleId }).then(() => page19.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page19 } } = this.props;
    page19.setDetailModalVisible(true);
    page19.setDetailData(record.users);
  }
    ...
    return (
      <div className="page19">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page19.tableData)}
          bordered />
