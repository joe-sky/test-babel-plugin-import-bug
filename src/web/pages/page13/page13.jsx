...
export default class Page13 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page13.getRoleMenuTree().then(() => page13.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page13.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page13 } } = this.props;
      const searchRole = page13.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page13.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page13 } } = this.props;
    page13.setAddModalVisible(true);
    page13.setDisable(true);
    page13.setActiveKey('tab1');
    page13.setAddInputRole('');
    page13.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page13.deleteRole({ roleId: roleId })
          ]).then(() => {
            page13.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page13 } } = this.props;
    page13.setEditModalVisible(true);
    page13.setSaveBtnDisabled(true);
    page13.setActiveKey('tab1');
    page13.setAddInputRole(record.name);
    page13.setAddInputDes(record.describe);
    page13.setRoleId(record.roleId);
    page13.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page13.getRoleMenuTree({ roleId: record.roleId }).then(() => page13.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page13 } } = this.props;
    page13.setDetailModalVisible(true);
    page13.setDetailData(record.users);
  }
    ...
    return (
      <div className="page13">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page13.tableData)}
          bordered />
