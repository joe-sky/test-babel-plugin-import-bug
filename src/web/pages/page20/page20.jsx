...
export default class Page20 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page20.getRoleMenuTree().then(() => page20.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page20.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page20 } } = this.props;
      const searchRole = page20.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page20.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page20 } } = this.props;
    page20.setAddModalVisible(true);
    page20.setDisable(true);
    page20.setActiveKey('tab1');
    page20.setAddInputRole('');
    page20.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page20.deleteRole({ roleId: roleId })
          ]).then(() => {
            page20.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page20 } } = this.props;
    page20.setEditModalVisible(true);
    page20.setSaveBtnDisabled(true);
    page20.setActiveKey('tab1');
    page20.setAddInputRole(record.name);
    page20.setAddInputDes(record.describe);
    page20.setRoleId(record.roleId);
    page20.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page20.getRoleMenuTree({ roleId: record.roleId }).then(() => page20.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page20 } } = this.props;
    page20.setDetailModalVisible(true);
    page20.setDetailData(record.users);
  }
    ...
    return (
      <div className="page20">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page20.tableData)}
          bordered />
