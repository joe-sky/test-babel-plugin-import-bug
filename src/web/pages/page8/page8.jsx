...
export default class Page8 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page8.getRoleMenuTree().then(() => page8.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page8.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page8 } } = this.props;
      const searchRole = page8.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page8.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page8 } } = this.props;
    page8.setAddModalVisible(true);
    page8.setDisable(true);
    page8.setActiveKey('tab1');
    page8.setAddInputRole('');
    page8.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page8.deleteRole({ roleId: roleId })
          ]).then(() => {
            page8.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page8 } } = this.props;
    page8.setEditModalVisible(true);
    page8.setSaveBtnDisabled(true);
    page8.setActiveKey('tab1');
    page8.setAddInputRole(record.name);
    page8.setAddInputDes(record.describe);
    page8.setRoleId(record.roleId);
    page8.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page8.getRoleMenuTree({ roleId: record.roleId }).then(() => page8.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page8 } } = this.props;
    page8.setDetailModalVisible(true);
    page8.setDetailData(record.users);
  }
    ...
    return (
      <div className="page8">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page8.tableData)}
          bordered />
