...
export default class Page16 extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([
      ...
      page16.getRoleMenuTree().then(() => page16.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page16.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page16 } } = this.props;
      const searchRole = page16.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page16.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page16 } } = this.props;
    page16.setAddModalVisible(true);
    page16.setDisable(true);
    page16.setActiveKey('tab1');
    page16.setAddInputRole('');
    page16.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page16.deleteRole({ roleId: roleId })
          ]).then(() => {
            page16.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page16 } } = this.props;
    page16.setEditModalVisible(true);
    page16.setSaveBtnDisabled(true);
    page16.setActiveKey('tab1');
    page16.setAddInputRole(record.name);
    page16.setAddInputDes(record.describe);
    page16.setRoleId(record.roleId);
    page16.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page16.getRoleMenuTree({ roleId: record.roleId }).then(() => page16.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page16 } } = this.props;
    page16.setDetailModalVisible(true);
    page16.setDetailData(record.users);
  }
    ...
    return (
      <div className="page16">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page16.tableData)}
          bordered />
