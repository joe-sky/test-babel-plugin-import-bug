...
export default class Page4 extends Component {
  ...
  componentDidMount() {
    const { store: { page4 } } = this.props;

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page4.getRoleManagementData(),
      page4.getRoleMenuTree().then(() => page4.initTree())
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onSearch() {
    if (this.inputRole == '') {
      const closeLoading = Message.loading('正在获取数据...', 0);
      Promise.all([
        this.props.store.page4.getRoleManagementData(),
      ]).then(() => {
        closeLoading();
      });
    } else {
      const { store: { page4 } } = this.props;
      const searchRole = page4.tableDataO.filter(n => n.name.indexOf(this.inputRole.trim()) > -1);
      page4.setTableData(searchRole);
    }
  }
  ...
  onAddRole() {
    const { store: { page4 } } = this.props;
    page4.setAddModalVisible(true);
    page4.setDisable(true);
    page4.setActiveKey('tab1');
    page4.setAddInputRole('');
    page4.setAddInputDes('');
  }
    ...
    if (this.selectedRowKeys.length == 0) {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          Promise.all([
            page4.deleteRole({ roleId: roleId })
          ]).then(() => {
            page4.getRoleManagementData();
          }).then(() => {
  ...
  onEdit(record, index) {
    const { store: { page4 } } = this.props;
    page4.setEditModalVisible(true);
    page4.setSaveBtnDisabled(true);
    page4.setActiveKey('tab1');
    page4.setAddInputRole(record.name);
    page4.setAddInputDes(record.describe);
    page4.setRoleId(record.roleId);
    page4.setDisable(false);

    const closeLoading = Message.loading('正在获取数据...', 0);
    Promise.all([
      page4.getRoleMenuTree({ roleId: record.roleId }).then(() => page4.initTree()),
    ]).then(() => {
      closeLoading();
    });
  }
  ...
  onDetail(record, index) {
    const { store: { page4 } } = this.props;
    page4.setDetailModalVisible(true);
    page4.setDetailData(record.users);
  }
    ...
    return (
      <div className="page4">
        ...
        <Table rowSelection={this.getRowSelection()}
          ...
          dataSource={toJS(page4.tableData)}
          bordered />
